...
const MessageUser = (props) => {
	// sockets
	const [prevRoom_id, setPrevRoom_id] = useState()
	const room__id = JSON.stringify(props.chatUser + JSON.parse(localStorage.getItem('user')).email)
	...
	const listChats = async () => {
		try {
			...
			socket.emit('leave-room', prevRoom_id)
			setPrevRoom_id(room__id)
	...
	const handleSubmit = async (e) => {
		e.preventDefault()
		try {
			const members = [JSON.parse(localStorage.getItem('user')).email, props.chatUser]
			const messages = [{
				message: message,
...
const MessageBox = (props) => {
	...
	const listChats = async () => {
		try {
			...
			setDocData(userchats.data.docData)
		} catch (error) {
			console.log(error)
		}
	}
	...
	return (
		<div className="message__box">
			...
			{chats.map((item) => {
				return (
					<div id={counter} onClick={event => {props.onChange(event.target.id, item.members[0], docData[event.target.id][0].name)}} style={props.isActive === -1 ? {backgroundColor: "#777777"} : {}} className="user__box" key={counter+1}>{docData && docData[counter++][0].name}</div>
				)
			})}
		</div>
	)
